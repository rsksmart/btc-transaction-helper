...
class BtcTransactionHelper {
    ...
    async transferBtc(senderAddressInformation, recipientsTransactionInformation, paymentData) {
        try {
            const totalAmountInBtc = recipientsTransactionInformation.reduce(
                (sum, output) => sum + output.amountInBtc,
                0
            );
    ...
    getOutputAddress(outputScript) {
        try {
            ...
            // Outputs with OP_RETURN do not have an address
            return null;
        }
    }
    ...
    async fundAddress(address, amountInBtc, mineBlock = true) {
        ...
        const btcTxHash = await this.nodeClient.fundAddress(address, fundAmount);
        if (mineBlock) {
            await this.mine();
        }
        return btcTxHash;
